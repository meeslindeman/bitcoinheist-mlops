...
def test_log_and_load_model():
    model, X = _tiny_trained_model()
    ...
    loaded_model = load_model_from_mlflow(run_id=run_id)
    assert isinstance(loaded_model, type(model))

    # note: loaded model gives same predictions on the tiny dataset
...
def test_get_latest_run_id_after_logging():
    model, _ = _tiny_trained_model()

    cv_scores = {"test_f1": np.array([0.3, 0.4])}
    test_report = {"0": {"precision": 1.0, "recall": 1.0, "f1-score": 1.0, "support": 1}}

    # note: this should become the latest run now
    ...
    latest_run_id = get_latest_run_id(RunConfig.run_name)

    assert latest_run_id is not None
    assert latest_run_id == new_run_id
